...
@section('title', isset($offer) ? 'Create Invoice from Offer' : 'Create Invoice')

@section('content')

	@include('partials.modals.add-customer', ['statuses' => $customerStatuses, 'countries' => $countries])
	...
	<div class="l-page-header">
		<div class="l-page-header__left-wrap">
			<h1 class="l-page-header__title">
				@if(isset($offer))
					Create Invoice from Offer #{{ $offer->offer_number }}
				@else
					Create Invoice
				@endif
			</h1>
		...
		<div class="l-page-header__right-wrap">
			<div class="l-page-header__right-content">
				<a href="{{ route('invoices.index') }}" class="e-button e-button--bordered">Cancel</a>
				<button type="submit" form="create-invoice-form" class="e-button">Create Invoice</button>
			</div>
		</div>
	</div>

	<div class="l-page-content l-page-content--inline-spacing">
		<form id="create-invoice-form" class="e-form" method="POST" action="{{ route('invoices.store') }}">
			@csrf

			{{-- Hidden fields for automatic values --}}
			<input type="hidden" name="invoice_date" value="{{ now()->format('Y-m-d') }}">
			<input type="hidden" name="status" value="{{ $defaultStatusId }}">
			@if(isset($offer))
					...
					<div class="l-grid__col">
						<div class="e-form__labels-inside">
							<div class="e-form__field">
								<div class="e-form__field-wrap" style="display: flex; gap: 1rem; align-items: flex-end;">
									<div style="flex: 1;">
										<x-form.select 
											name="customer_id" 
											label="Customer"
											:options="$customers"
											:value="old('customer_id', isset($offer) ? $offer->customer_id : null)"
											placeholder="Select a customer"
											required
										/>
									</div>
									<div>
										<button type="button" class="e-button e-button--bordered" open-modal-id="add-customer" style="margin-bottom: 0; height: 4.8rem;">
											@svg('plus')
										</button>
									</div>
								</div>
			...
			<x-form.harmonica title="Assignment" subtitle="Briefly describe what your assignment entails.">
				...
				<div class="e-form__labels-inside">
					<x-form.textarea 
						name="intro" 
							label="Introduction" 
						:value="old('intro', isset($offer) ? $offer->intro : '')"
					...
					<x-form.textarea 
						name="desc" 
						label="Description" 
						:value="old('desc', isset($offer) ? $offer->desc : '')"
			...
			<x-form.harmonica title="Invoice lines" subtitle="List the products and/or services you are invoicing." class="{{ isset($existingItems) && count($existingItems) > 0 ? 'c-harmonica--is-open' : '' }}">
			...
			<x-form.harmonica title="Intellectual Property" subtitle="Define intellectual property transfer terms for this invoice.">
				<div class="e-form__labels-inside">
					...
					<div class="ip-terms-preview" id="ip-terms-preview" style="display: none;">
						<div class="ip-terms-preview__title">Preview:</div>
						<div class="ip-terms-preview__content" id="ip-terms-content"></div>
					</div>
				</div>
			</x-form.harmonica>
			...
			<x-form.harmonica title="Notes" subtitle="Additional notes for internal use or to show on the invoice.">
				...
				<div class="e-form__labels-inside">
					<x-form.textarea 
						...
						:value="old('notes', isset($offer) ? $offer->notes : '')"
	...
	<script>
		...
		// Existing items (from offer or edit)
		var existingItems = @json(isset($existingItems) ? $existingItems : []);
		...
		(function() {
			function initIpTransferPreview() {
				...
				if (select && preview && content) {
					...
					// Check initial value
					if (select.value && ipTransferTexts[select.value]) {
						content.textContent = ipTransferTexts[select.value];
						preview.style.display = 'block';
					}
				}
			}
