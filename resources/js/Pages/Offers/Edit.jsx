...
export default function Edit({
    ...
    const form = useForm({
        ...
        offer_date: offer.offer_date?.slice?.(0, 10) || offer.offer_date || '',
        valid_until: offer.valid_until?.slice?.(0, 10) || offer.valid_until || '',
        intro: offer.intro || '',
        desc: offer.desc || '',
        notes: offer.notes || '',
        status: offer.status || '',
    ...
    return (
        <AuthenticatedLayout title="Edit offer">
            <h1 className="mb-6 text-2xl font-semibold">Edit {offer.offer_number}</h1>
            <form
                onSubmit={(e) => {
                    e.preventDefault();
                    form.transform((data) => ({
