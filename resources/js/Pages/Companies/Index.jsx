...
export default function Index({ companies = [], requireCompanyApproval }) {
    ...
    return (
        <AuthenticatedLayout title={t('companies.title')}>
            <div className="space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className="text-3xl font-semibold tracking-tight text-slate-900">{t('companies.title')}</h1>
                        <p className="mt-1 text-sm text-slate-500">
                            {requireCompanyApproval ? t('companies.subtitle_approval') : t('companies.subtitle')}
                        </p>
                    </div>
                    <Link
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <StatCard label={t('common.pending')} value={stats.pending} />
                ...
                <section className="rounded-3xl border border-slate-200 bg-white">
                    <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 sm:px-6">
                        <div className="flex items-center gap-2">
                            <h2 className="text-lg font-semibold text-slate-900">{t('companies.yours')}</h2>
                        ...
                        <table className="w-full min-w-[720px] text-left text-sm">
                            <thead>
                                <tr className="border-y border-slate-100 text-[11px] uppercase tracking-wide text-slate-400">
                                    <th className="px-5 py-3 font-medium sm:px-6">{t('companies.title')}</th>
                                    <th className="px-3 py-3 font-medium">{t('common.email')}</th>
                                    <th className="px-3 py-3 font-medium">{t('companies.city')}</th>
                                    <th className="px-3 py-3 font-medium">{t('common.status')}</th>
                                    <th className="px-5 py-3 text-right font-medium sm:px-6">{t('common.active')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {rows.length === 0 && (
                                ...
                                {rows.map((company) => {
                                    const status = company.status_relation?.name || (company.is_active ? t('common.active') : t('common.inactive'));
                                    return (
                                        <tr key={company.id} className="border-b border-slate-50 last:border-0">
                                            ...
                                            <td className="px-5 py-4 text-right sm:px-6">
                                                <StatusPill tone={company.is_active ? 'paid' : 'draft'}>{company.is_active ? t('common.yes') : t('common.no')}</StatusPill>
                                            </td>
                                        </tr>
                                    );
                                })}
